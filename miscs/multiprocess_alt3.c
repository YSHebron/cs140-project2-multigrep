...
void routine(struct proc_data *data, int N) {
    ...
    while (1) {
        ...
        fptr = fopen(busy_procs, "r+");
        fd = fileno(fptr);
            flock(1, LOCK_EX);printf("[%d] Got here A\n", ID);flock(1, LOCK_UN);

        tqfd = open(task_queue, O_RDONLY);
        while (read(tqfd, taskpath, MAXPATH) == 0) {
            flock(fd, LOCK_EX);
        ...
        while ((d = readdir(dirp)) != NULL) {   // loop until end of dir stream is reached
            ...
            if (isDir(pathbuff)) {
                tqfd = open(task_queue, O_WRONLY);
                write(tqfd, pathbuff, MAXPATH);
                close(tqfd);
...
int main(int argc, char *argv[]) {
    ...
    char * task_queue = "/tmp/task_queue";
    unlink(task_queue);   // delete "task_queue" if it already exists from a previous run
    printf("%s\n", task_queue);
    pdata.task_queue = task_queue;
    ...
    // NOTE: Opening a FIFO for reading normally blocks until some other process opens the same FIFO for writing, and vv.

    int tqfd = open(task_queue, O_WRONLY);
    write(tqfd, rootpath, MAXPATH);
    close(tqfd);
